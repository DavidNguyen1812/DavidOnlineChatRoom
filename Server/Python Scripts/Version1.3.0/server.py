...
def send_message_to_clients(server_message_to_be_sent, clientAddress):
    ...
    if server_message_to_be_sent.startswith("The digital signature verification from the server failed"):
        ...
        if server_message_to_be_sent.startswith("#0010"):
            ...
            for msgID in active_clients[clientAddress][5]:
                ...
                counter += 1
                signing_key = RSA.import_key(open(SERVERPRIVATEKEY, 'rb').read())
                hashed_msg = SHA256.new(server_message_to_be_sent.encode())
                server_signature = pss.new(signing_key).sign(hashed_msg)
                server_message_to_be_sent = f"{server_message_to_be_sent}:{base64.b64encode(server_signature).decode('ascii')}"
                send_length = str(len(server_message_to_be_sent)).encode(DECODE_FORMAT)
                send_length += b' ' * (HEADER - len(send_length))
                clientSocket.send(send_length)
                clientSocket.send(server_message_to_be_sent.encode(DECODE_FORMAT))
    ...
    try:
        while True:
            ...
            if client_msg_length:
                ...
                print(f"Incoming messsage from client [{clientAddress}]: {client_msg}\nClient Status: {active_clients[clientAddress][2]}")
                if not active_clients[clientAddress][2] == 'First Exchange':
                    verifier_key = RSA.import_key(open(active_clients[clientAddress][3], 'rb').read())
                    client_signature = client_msg.split(":")[len(client_msg.split(":")) - 1]
                    hashed_msg = SHA256.new(client_msg.split(f":{client_signature}")[0].encode())
                    client_msg = client_msg.split(f":{client_signature}")[0]
                    client_signature = base64.b64decode(client_signature)
                    verifier = pss.new(verifier_key)
                    try:
                        verifier.verify(hashed_msg, client_signature)
                    except ValueError:
                        print(f"Signature verification failed for client [{clientAddress}]: {client_msg} failed. Terminating connection!")
                        send_message_to_clients(f"The digital signature verification from the server failed, this indicate"
                                               f" a potential MIMT in the communication channel trying to tamper with the message exchange between"
                                               f" you and the server. Your connection to the server has been terminated for reduction in security risk!", clientAddress)
                        client_msg = "#0100"
                else:
                    active_clients[clientAddress][2] = ''
...
def start():
    ...
    try:
        while True:
            ...
            active_clients[clientAddress[1]] = [clientSocket, '', 'First Exchange', '', 0]  # [clientSocket, username, status, public key, cursing record]
