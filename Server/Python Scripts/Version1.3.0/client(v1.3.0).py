...
def sendMessageToServer(msg):
    ...
    if STATUS != "FIRST EXCHANGE":
        ...
        msg = f"{tag}{msg}"
        signing_key = RSA.import_key(open(CLIENTPRIVATEKEY, 'rb').read())
        hashed_msg = SHA256.new(msg.encode())
        client_signature = pss.new(signing_key).sign(hashed_msg)
        msg = f"{msg}:{base64.b64encode(client_signature).decode('ascii')}"
    else:
        STATUS = "AUTHENTICATING"
    ...
    while True:
        if STATUS != "IN SESSION":
            ...
            if server_msg_length:
                ...
                verifier_key = RSA.import_key(open(SERVERPUBLICKEY, 'rb').read())
                server_signature = server_msg.split(":")[len(server_msg.split(":")) - 1]
                hashed_msg = SHA256.new(server_msg.split(f":{server_signature}")[0].encode())
                server_msg = server_msg.split(f":{server_signature}")[0]
                server_signature = base64.b64decode(server_signature)
                verifier = pss.new(verifier_key)
                try:
                    verifier.verify(hashed_msg, server_signature)
                except ValueError:
                    updateMessageBox(f"The digital signature verification from the server failed, this indicate"
                                     f" a potential MIMT in the communication channel trying to tamper with the message exchange between"
                                     f" you and the server. Your connection to the server has been terminated for reduction in security risk!")
                ...
                if server_msg.startswith("LASTMESSAGE"):
                    server_msg = server_msg.split("LASTMESSAGE")[1]
                    terminate = True
        ...
        while True:
            if listening:
                ...
                if server_msg_length:
                    server_msg_length = int(server_msg_length)
                    server_msg = clientSocket.recv(server_msg_length).decode(DECODE_FORMAT)
                    if server_msg.startswith("The digital signature verification from the server failed"):
                        userinput_textbox.config(state=DISABLED)
                        send_button.config(state=DISABLED)
                        discconection_button.config(state=DISABLED)
                    else:
